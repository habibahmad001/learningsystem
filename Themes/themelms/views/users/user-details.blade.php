...
<?php
                                $temp="";
                                if (checkRole(getUserGrade(4)) && $record->id != \Auth::user()->id) {
                                    $temp .= '<div><form action="'.URL_AUTHADMIN_LOGIN.'" method="POST"><input type="hidden" name="slug"  id="slug_'.$record->id.'" value="'.$record->id.'"><input name="_token" value="'.csrf_token().'" type="hidden">';
                                    $temp .= '<a class="btn btn-success" onclick="this.closest(\'form\').submit();return true;" ><i class="fa fa-user-secret"></i>  ' . getPhrase("Login as User") . '</a>';
                                    $temp .= '</form></div>';
                              }
                                echo $temp;
				 	...
				 	<div class="col-md-6 col-sm-6">
				 		...
				 		<div class="media state-media box-ws">
				 			...
				 			<div class="media-body">
								...
								<a href="{{URL_PAYMENTS_LIST.$record->slug}}">{{ getPhrase('subscriptions/courses')}}</a>
				 			</div>
				 		</div>
				 	</div>
