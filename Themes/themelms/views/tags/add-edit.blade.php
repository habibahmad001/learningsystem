				...
				<div class="row">
					<div class="col-lg-12">
						<ol class="breadcrumb">
							...
							<li><a href="{{URL_TAGS}}">{{ getPhrase('Tags')}}</a></li>
				...
				<div class="panel panel-custom col-lg-6 col-lg-offset-3" ng-init="initAngData('{{ $settings }}');" ng-controller="angLmsController">
					<div class="panel-heading"> 
						<div class="pull-right messages-buttons">
							<a href="{{URL_TAGS}}" class="btn  btn-primary button" >{{ getPhrase('list')}}</a>
						</div>
					<h1>{{ $title }}  </h1>
					</div>
					...
					@if ($record)
					 <?php $button_name = getPhrase('update'); ?>
						{{ Form::model($record, 
						array('url' => URL_TAGS_EDIT. $record->id, 'novalidate'=>'','name'=>'formLms ',
						'method'=>'patch', 'files' => true)) }}
					@else
						{!! Form::open(array('url' => URL_TAGS_ADD,
							'novalidate'=>'','name'=>'formLms ',
						'method' => 'POST', 'files' => true)) !!}
					@endif
					 @include('tags.form_elements',
