...
class QuizCategoryController extends Controller
{
    ...
    public function getDatatable()
    {
        ...
        ->editColumn('image', function($records){
            ...
            $image_path = IMAGE_PATH_UPLOAD_QUIZ_DEFAULT;
            if($records->image)
                $image_path = IMAGE_PATH_UPLOAD_QUIZ_CATEGORIES.$records->image;
    ...
    public function update(Request $request, $slug)
    {
    	...
    	$record = QuizCategory::getRecordWithSlug($slug);
		$rules = [
         'category'          => 'bail|required|min:3|max:60',
         ...
         if ($request->hasFile($file_name)) {
          ...
          $fileName = $record->id.'-'.$file_name.'.'.$request->$file_name->guessClientExtension();
             ...
             if(env('FILESYSTEM_DRIVER')=='s3') {
                 ...
                 uploadToS3($image_normal, 'exams/categories/', $fileName);
             }else {
