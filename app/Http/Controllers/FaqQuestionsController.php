    ...
    */
    ...
    public function getDatatable($slug = '')
    {
        ...
        $records = Faq::join('faqcategories', 'faqs.category_id', 'faqcategories.id')
        				->select(['faqcategories.category', 'faqs.question', 'faqs.status','faqs.slug','faqs.id as faqid'])
        ...
        return Datatables::of($records)
            ->addColumn('check_course', function ($records) {
                ...
                $link_data1 = '<input name="checkbox_course[]" type="checkbox" class="delete_check" id="delcheck_'.$records->faqid.'"    value="'.$records->faqid.'">';
        ...
        ->editColumn('status', function($records)
        {
            ...
            ->rawColumns([ 'status','action','check_course'])
            ->removeColumn('slug')
            ->removeColumn('faqid')
        ->make();
...
//
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        $records_obj = Faq::whereIn('id', $id_array);
        try {
            if (!env('DEMO_MODE')) {
