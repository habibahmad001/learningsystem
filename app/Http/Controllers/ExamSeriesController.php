      ...
      if(!checkRole(getUserGrade(2)))
      {
       ...
       // $records = array();
        $records = ExamSeries::join('quizcategories', 'examseries.category_id', '=', 'quizcategories.id')
            ->select(['examseries.id as id','examseries.title as title','examseries.image as image', 'examseries.is_paid as is_paid', 'quizcategories.category as category', 'examseries.cost as cost', 'examseries.validity as validity','examseries.total_exams as total_exams','examseries.total_questions as total_questions','examseries.slug as slug','examseries.updated_at as updated_at' ])
            ->orderBy('examseries.updated_at', 'desc');
...
//            ->orderBy('updated_at', 'desc');
        ...
        ->editColumn('is_paid', function($records)
        {
            ...
            ->rawColumns(['image', 'title', 'is_paid','action'])
            ->removeColumn('id')
        ->removeColumn('slug')
    ...
    public function store(Request $request)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        $name  						=  $request->title;
        $slug=str_slug($name);
        $slugrecord = ExamSeries::select(['slug'])->where('slug','=',$slug)->get();
if($slugrecord){
    $slug=$slug.'-'.$request->exam_type.'-'.$request->category_id.'-'.rand(10,99);
}
...
//        }
    	return json_encode(array('exams'=>$exams,'series_return'=>$exam_series));
    }
