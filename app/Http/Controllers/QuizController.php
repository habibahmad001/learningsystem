...
class QuizController extends Controller
{
    ...
    public function getDatatable($slug = '')
    {
      ...
      if(!checkRole(getUserGrade(2)))
      {
        ...
        if($slug=='')
        {
            ...
            ->select(['quizzes.id as qid','title', 'dueration', 'category', 'is_paid', 'total_marks','exam_type','tags','quizzes.slug' ])
        ...
        return Datatables::of($records)
            ->addColumn('check_course', function ($records) {
                ...
                $link_data1 = '<input name="checkbox_course[]" type="checkbox" class="delete_check" id="delcheck_'.$records->qid.'"    value="'.$records->qid.'">';
         ...
         ->editColumn('exam_type',function($records)
        {
           return App\ExamType::where('code',$records->exam_type)->first()->title;
            ...
            ->rawColumns(['title', 'is_paid','action','check_course'])
    ...
    public function create()
    {
      ...
      $data['record']             = FALSE;
      $data['active_class']       = 'exams';
      $data['categories']         = Arr::pluck(QuizCategory::all(), 'category', 'id');
      $data['instructions']       = Arr::pluck(App\Instruction::all(), 'title', 'id');
     ...
     //dd($request);
      ...
      $rules = [
        ...
        if($request->show_in_front == 1){
             ...
             if($request->is_paid == 1){
                 flash('warning','practice_exam_must_be_non_paid_exam', 'warning');
             // flash('Ooops...!','practice_exam_must_be_non_paid_exam','overlay');
               //  return redirect(URL_QUIZ_ADD);
               return back();
             }
             elseif ($request->exam_type !='NSNT') {
                 flash('warning','practice_exam_must_be_no_section_no_timer_exam', 'warning');
//              flash('Ooops...!','practice_exam_must_be_no_section_no_timer_exam','overlay');
               return back();
                 //return redirect(URL_QUIZ_ADD);
             }
...
//try{
        ...
        if ($request->hasFile($file_name))
        {
            ...
            $filename_new    = $this->processUpload($request, $record,$file_name);

            $record->image  = $filename_new;
              $record->save();
...
//        }
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        $record = Quiz::whereIn('id', $id_array);
        try{
            if(!env('DEMO_MODE')) {
     ...
     public function processUpload(Request $request, $record, $file_name)
     {
         ...
         if ($request->hasFile($file_name)) {
          //$examSettings = getExamSettings();
             $examSettings = getSettings('exams');
          $destinationPath      = $examSettings->seriesImagepath;
             $imageObject = new ImageSettings();
          $fileName = $record->id.'-'.$file_name.'_'.time().'.'.$request->$file_name->guessClientExtension();
...
//                 $image_thumb = Image::make($file->getRealPath())->fit($imageObject->getThumbnailSize());
                 ...
                 uploadToS3($image_normal,'exams/series/',$fileName);
