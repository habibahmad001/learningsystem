    ...
    public function getDatatable($slug = '')
    {
      ...
      if(!checkRole(getUserGrade(2)))
      {
        ...
        $records = array();
            ...
            $records = Couponcode::where("id", "!=", 0)
            ->select(['title', 'coupon_code', 'discount_type', 'discount_value', 'status', 'id','slug' ])
...
//        $record->discount_maximum_amount = $request->discount_maximum_amount;
        ...
        $record->coupon_code_applicability = $request->applicability;
        if(isset($request->forselectedcourses)) {
            $record->selectcourses  = json_encode(implode(',', $request->coursename));
        } else {
            $record->selectcourses  = "";
        }
        $record->save();
