  ...
  public $columns = '';
    ...
    public function getDatatable(Request $request)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        ->rawColumns(['subject_title','action','check_course'])
        ->removeColumn('id')
        ->removeColumn('slug')
        ->removeColumn('is_lab')
    ...
    public function getQuestions($slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        ->select(['subject_title', 'topic_name',
            ...
            'questionbank.difficulty_level', 'questionbank.id as qid', 'questionbank.slug',
            'questionbank.updated_at'])
        ->where('questionbank.subject_id','=', $subject->id)
        //->where('questionbank.record_updated_by','=', Auth::user()->id)
        ->orderBy('updated_at','desc');
        ...
        $table = Datatables::of($records)->removeColumn('slug')
            ...
            ->rawColumns(['question','action','check_course'])
        ->removeColumn('id')
        ->removeColumn('slug')
        ->removeColumn('updated_at')
     ...
     catch(Exception $e)
     {
      DB::rollBack();
       ...
       if(getSetting('show_foreign_key_constraint','module'))
       {
          ...
          flash('oops...!','improper_data_in_the_question :: '.$e->getMessage(), 'error');
       }
     }
     ...
     // dd($request);
      ...
      try{
       ...
       if($request->question_type == 'para') {
        ...
        $name 					        = $request->question;
        $record->question 			= $name;
        $record->slug 			    = str_slug(getHashCode());
        $record->subject_id			= $request->subject_id;
        $record->topic_id				= $request->topic_id;
        $record->question				= $request->question;
        $record->difficulty_level		= $request->difficulty_level;
         ...
         if ($request->hasFile($file_name)) {
          ...
          if($type!='option')
          {
             ...
             if(env('FILESYSTEM_DRIVER')=='s3'){
                 ...
                 $image_normal = Image::make($file->getRealPath());

                 uploadToS3($image_normal,'exams/',$fileName);
             }else {
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        $id_array = $request->input('deleteids_arr');
         ...
         $records = QuestionBank::whereIn('id', $id_array);
        try{
            if(!env('DEMO_MODE')) {
    ...
    {
        $id_array = $request->input('deleteids_arr');
        $record = Subject::whereIn('id', $id_array);
        try{
            if(!env('DEMO_MODE')) {
    ...
    public function readExcel(Request $request)
    {
        ...
        try{
            if($request->hasFile('excel')) {
                ...
                $import = Excel::import(new SubjectsImport, $file);

            }
            flash('success','record_imported_successfully', 'success');
