...
class InstructorSettingController extends Controller
{
    ...
    {
        $data['active_class'] = 'payoutsettings';
        $data['title'] = getPhrase('payout settings');
        $user = User::where('id', Auth::User()->id)->first();
        $data['user']=$user;

        return view(getTheme() . '::instructors.settings.pay_settings',$data);
    ...
    public function instructorprofile($id)
    {
        ...
        return view('front.instructor', compact('user', 'course'));
    }
}
