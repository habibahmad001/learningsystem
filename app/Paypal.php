...
//	var $setting='';
	...
	{
//		 $settings1 = Settings::where('key', '=', 'paypal')->first();
//		 $settings = json_decode($settings->settings_data);
//dd(getSetting('email','paypal'));
		#Account information
		$config["business"] 	= getSetting('email','paypal'); //Account email or id
//		$config["business"] 	= 'sheensoltech123@gmail.com';//$settings->email; //Account email or id
		$config["cmd"] 			= '_cart'; //Do not modify
		$config["production"] 	= false;

		if(getSetting('account_type','paypal') =='live')
		...
		$config["return"] 			= URL_PAYPAL_PAYMENT_SUCCESS; //The URL to which PayPal redirects buyers browser after they complete their payments.
		...
		$config["no_shipping"] = ''; //Languaje [EN,ES]
	...
	public function intilizeConfig()
	{
		...
		$this->config['business'] 			= getSetting('email','paypal');
		$this->config['currency_code'] 		= getSetting('currency','paypal');
